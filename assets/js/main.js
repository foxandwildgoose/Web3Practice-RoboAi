(() => {
  ...
  function swapThemeAssets(theme){
    try{
      ...
      const fav = document.querySelector('link[rel="icon"]#favicon');
      if (fav){
        const d = fav.getAttribute('data-dark-href'), l = fav.getAttribute('data-light-href');
        if (d && l) fav.href = (theme==='dark') ? d : l;
      }
    }catch(e){}
  }
        ...
        if (!applyTheme._bound) {
          mq.addEventListener('change', () => {
            ...
            if (pref === 'system') {
              ...
              swapThemeAssets(mq.matches ? 'dark':'light');
            }
          });
